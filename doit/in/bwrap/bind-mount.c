            ...
            char        **failing_path)
{
  ...
  if (src)
    {
      if (mount (src, dest, NULL, MS_SILENT | MS_BIND | MS_SLAVE | (recursive ? MS_REC : 0), NULL) != 0)
  ...
  new_flags = current_flags | (devices ? 0 : MS_NODEV) | MS_SLAVE | MS_NOSUID | (readonly ? MS_RDONLY : 0);
  if (new_flags != current_flags &&
      mount ("none", resolved_dest,
             NULL, MS_SILENT | MS_BIND | MS_REMOUNT | MS_SLAVE | new_flags, NULL) != 0)
    {
      if (failing_path != NULL)
        *failing_path = steal_pointer (&resolved_dest);
