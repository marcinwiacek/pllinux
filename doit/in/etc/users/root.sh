...
export PS1="\e[31m[\t] root:\w\$(if [ -f "/app/git/current/git" ] && /app/git/current/git rev-parse --git-dir > /dev/null 2>&1; then echo -n \" \e[33m\";/app/git/current/git branch --show-current; fi) # \e[m"
sh
